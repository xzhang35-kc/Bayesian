...
setwd("C:/ZXiao/MVP-Ordinal-P1/Figure1/PX-GS-Simu-PCS/Simu500S5D-UX")
...
################PMDA#################################################################################################
#####################################################################################################################
setwd("C:/ZXiao/MVP-Ordinal-P1/Figure1/PX-GSM-Simu-PCS/Simu500S5D-UX")
#Beta <- matrix(scan("CorrOrdBeta.dat"),ncol=2,byrow=T)
#R <- matrix(scan("CorrOrdR.dat"),ncol=25,byrow=T)
#WR <- matrix(scan("CorrOrdSig.dat"),ncol=25,byrow=T)
#Gama <- matrix(scan("CorrOrdGama.dat"), ncol=20, byrow=T)
###################################################################################################################
####setwd("C:/Users/kuiz-adm/iCloudDrive/ZXiao")###################################################################
###################################################################################################################
###setwd("C:/Users/kuiz-adm/iCloudDrive/ZXiao")####################################################################
###################################################################################################################
remove_col <- function(file_names, out_names, num_rows) {
	# define a function
	find_na <- function(b) {
		a <- TRUE
		if(any(b == "-1.#IND"))
	...
	N <- length(file_names)
	in_dat <- as.list(1:N)
	na_index <- as.list(1:N)
	out_dat <- as.list(1:(N + 1))
	for(n in 1:N) {
		in_dat[[n]] <- scan(file_names[n], what = "")
		in_dat[[n]] <- matrix(in_dat[[n]], nrow = num_rows[n])
		na_index[[n]] <- apply(in_dat[[n]], MARGIN = 2, find_na)
	}
	final_index <- na_index[[1]]
	for(n in 2:N) {
		final_index <- final_index & na_index[[n]]
	}
	for(n in 1:N) {
		out_dat[[n]] <- as.numeric(in_dat[[n]][, final_index])
	}
	out_dat[[N + 1]] <- sum(final_index)
	names(out_dat) <- c(out_names, "num_dat")
	out_dat
}
file_names <- c("CorrOrdBeta.dat", "CorrOrdGama.dat", "CorrOrdR.dat", "CorrOrdSig.dat")
out_names <- c("Beta", "Gama", "OrdR", "OrdSig")
S <- 10000
num_rows <- c(2 * S, 20* S, 25 * S, 25 * S)
out_dat <- remove_col(file_names, out_names, num_rows) 
str(out_dat)
NumSamp = out_dat$num_dat
Beta = matrix(out_dat$Beta,ncol=2,byrow=T)
Gama = matrix(out_dat$Gama, ncol=20, byrow=T)
R = matrix(out_dat$OrdR, ncol=25,byrow=T)
WR = matrix(out_dat$OrdSig, ncol=25,byrow=T)
...
setwd("C:/ZXiao/MVP-Ordinal-P1/Figure1/PX-MH-Simu-PCS/Simu500S5D-UX")
Beta <- matrix(scan("CorrOrdBeta.dat"),ncol=2,byrow=T)
R <- matrix(scan("CorrOrdR.dat"),ncol=25,byrow=T)
