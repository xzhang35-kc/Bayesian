...
int MHsampler(gsl_matrix *SD, gsl_matrix * WR, gsl_matrix * R, gsl_matrix * Z, gsl_matrix * XB, gsl_matrix * Psigma, int N, int pm, int m);
...
void Gibbsampler(gsl_matrix * Y, gsl_matrix * X, gsl_matrix * YY, gsl_matrix ** XX, gsl_matrix * Psigma, gsl_matrix ** Gama, gsl_matrix * JJ, int N, int K, int rep, int Inum, int Inump, int m, int pm, gsl_matrix * b, gsl_matrix * C,
...
int main(void)
{
	...
	Gibbsampler(Y, X, YY, XX, Psigma, Gama, JJ, N, K, rep, Inum, Inump, m, pm, b, C, outBetaFile, outZFile, outSigmaFile, outRFile, outGamaFile);
	// free memory;
	gsl_matrix_free(X);
	gsl_matrix_free(Y);
	gsl_matrix_free(YY);
				 ...
				 char *outBetaFile, char *outZFile, char *outSigmaFile, char *outRFile, char * outGamaFile)
{
	...
	for(i = 1; i <= Inum; i++)
	{
       ...
       for(j = 1; j <= Inump; j++)
       {
          sp = sp + MHsampler(SD, WR, newR, newZ, XB, Psigma, N, pm, m);
       }
