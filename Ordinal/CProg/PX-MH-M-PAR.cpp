...
int MHHsampler(gsl_matrix *SD, gsl_matrix * WR, gsl_matrix * R, gsl_matrix * Z, gsl_matrix * XB, gsl_matrix * Psigma, int N, int pm, int m);
void SampleW(gsl_matrix * Z, gsl_matrix * sigma, gsl_matrix * XB, gsl_matrix * Y, int N, gsl_matrix * W, gsl_matrix * D, gsl_matrix ** Gama, gsl_matrix * JJ);
...
double Posterior(int N, gsl_matrix * R, gsl_matrix * D, gsl_matrix * W, gsl_matrix * XB);
void InvWishart(gsl_matrix * SigmaMat, int m, gsl_matrix * ScaleMat);
int MHsampler(gsl_matrix * WR, gsl_matrix * R, gsl_matrix * D, gsl_matrix * W, gsl_matrix * XB, gsl_matrix * Psigma, int N, int m, int m0);
void Gibbsampler(gsl_matrix * Y, gsl_matrix * X, gsl_matrix * YY, gsl_matrix ** XX, gsl_matrix * Psigma, gsl_matrix ** Gama, gsl_matrix * JJ, int N, int K, int rep, int Inum, int Inump, int m, int m0, int pm, gsl_matrix * b, gsl_matrix * C,
...
int main(void)
{
	...
	Gibbsampler(Y, X, YY, XX, Psigma, Gama, JJ, N, K, rep, Inum, Inump, m, m0, pm, b, C, outBetaFile, outZFile, outRFile, outWRFile, outGamaFile);
...
void SampleW(gsl_matrix * Z, gsl_matrix * sigma, gsl_matrix * XB, gsl_matrix * Y, int N, gsl_matrix * W, gsl_matrix * D, gsl_matrix ** Gama, gsl_matrix * JJ)
{
    ...
    for(i = 0;i < N; i++)
	{
		for(j = 0; j < m; j++)
		{
			a = gsl_matrix_get(Z, i * m + j,0);
			b = gsl_matrix_get(D, j, j);
			gsl_matrix_set(W, i * m + j, 0, a*sqrt(b));
		}
	}
	// free matrix;
...
int MHHsampler(gsl_matrix *SD, gsl_matrix * WR, gsl_matrix * R, gsl_matrix * Z, gsl_matrix * XB, gsl_matrix * Psigma, int N, int pm, int m)
{
	...
	p1 = Priornew(m, Psigma, DcRDc);
	p2 = Priornew(m, Psigma, DRD);
	pro1 = Priornew(pm, Wcc, WR);
	pro2 = Priornew(pm, Wrr, Wc);
...
double Posterior(int N, gsl_matrix * R, gsl_matrix * D, gsl_matrix * W, gsl_matrix * XB)
{
	 ...
	 for(i = 0;i < N; i++)
	{
		for(j = 0; j < k; j++)
		{
			...
			b = gsl_matrix_get(D, j, j);
			gsl_matrix_set(Z, i * k + j, 0, a/sqrt(b));
		}
	}
...
int MHsampler(gsl_matrix * WR, gsl_matrix * R, gsl_matrix * D, gsl_matrix * W, gsl_matrix * XB, gsl_matrix * Psigma, int N, int m, int m0)
{
	int k, i;
	k = R -> size1;
	double a, u, p1, p2, post1, post2, pro1, pro2;
	double rho, df;
	gsl_matrix * Wc = gsl_matrix_alloc(k, k);
	...
	post1 = Posterior(N, Rc, Dc, W, XB);
	post2 = Posterior(N, R, D, W, XB);
				 ...
				 char *outBetaFile, char *outZFile, char *outRFile, char *outWRFile, char * outGamaFile)
{
	...
	gsl_matrix *newW = gsl_matrix_alloc(N * rep, 1);
    ...
    gsl_matrix *newD = gsl_matrix_alloc(rep, rep);
	...
	for(i = 1; i <= Inum; i++)
	{
       ...
       sum = sum + MHsampler(newWR, newR, newD, newW, XB, Psigma, N, m, m0);
       //SampleSD(SD, newR, Psigma, m0);
       //gsl_matrix_memcpy(SD, newSig);
       gsl_matrix_memcpy(WR, newWR);
       sp=0;
       for(j = 1; j <= Inump; j++)
       {
          sp = sp + MHHsampler(SD, WR, newR, newZ, XB, Psigma, N, pm, m0);
       }

       SampleW(newZ, newR, XB, Y, N, newW, SD, newGama, JJ);
